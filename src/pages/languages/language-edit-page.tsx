import { useState, useEffect } from "react";
import { Link, useParams, useNavigate } from "react-router-dom";
import { Pencil, ArrowLeft, Code, Loader2 } from "lucide-react";
import { useLanguageById, useUpdateLanguage } from "@/hooks/use-languages";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
	Card,
	CardContent,
	CardDescription,
	CardHeader,
	CardTitle,
} from "@/components/ui/card";

export default function LanguageEditPage() {
	const { id } = useParams<{ id: string }>();
	const navigate = useNavigate();
	const languageId = id ? Number(id) : NaN;
	const { data: language, isLoading, isError } = useLanguageById(languageId);
	const updateLanguage = useUpdateLanguage();

	const [code, setCode] = useState("");

	useEffect(() => {
		if (language) setCode(language.code);
	}, [language]);

	const handleSubmit = (e: React.FormEvent) => {
		e.preventDefault();
		if (Number.isNaN(languageId)) return;
		updateLanguage.mutate(
			{ id: languageId, language: { code } },
			{ onSuccess: () => navigate(`/languages/${languageId}`) }
		);
	};

	if (!id || Number.isNaN(languageId)) {
		return (
			<div className="space-y-6">
				<p className="text-destructive">Geçersiz dil ID.</p>
				<Button variant="outline" onClick={() => navigate("/languages")}>
					Listeye dön
				</Button>
			</div>
		);
	}

	if (isLoading || !language) {
		return (
			<div className="space-y-6">
				<div className="h-10 w-56 bg-muted animate-pulse rounded-lg" />
				<Card>
					<CardContent className="pt-6">
						<div className="h-10 w-full max-w-md bg-muted animate-pulse rounded" />
					</CardContent>
				</Card>
			</div>
		);
	}

	if (isError) {
		return (
			<div className="space-y-6">
				<p className="text-destructive">Dil bulunamadı veya bir hata oluştu.</p>
				<Button variant="outline" onClick={() => navigate("/languages")}>
					Listeye dön
				</Button>
			</div>
		);
	}

	return (
		<div className="space-y-8">
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div className="flex items-center gap-4">
					<Button variant="outline" size="icon" asChild className="shrink-0">
						<Link to={`/languages/${language.id}`}>
							<ArrowLeft className="size-4" />
						</Link>
					</Button>
					<div className="flex items-center gap-3 min-w-0">
						<div className="flex size-12 items-center justify-center rounded-xl bg-primary/10 text-primary shrink-0">
							<Pencil className="size-6" />
						</div>
						<div className="min-w-0">
							<h1 className="text-2xl font-bold tracking-tight truncate">
								Dili düzenle
							</h1>
							<p className="text-muted-foreground text-sm mt-0.5">
								{language.code} bilgisini güncelleyin
							</p>
						</div>
					</div>
				</div>
			</div>

			<Card className="w-full border-border/60 shadow-sm overflow-hidden">
				<CardHeader className="border-b border-border/60 bg-muted/30">
					<CardTitle className="text-lg">Dil bilgisi</CardTitle>
					<CardDescription>
						Dil kodunu güncelleyin
					</CardDescription>
				</CardHeader>
				<CardContent className="p-6">
					<form onSubmit={handleSubmit} className="space-y-6">
						<div className="max-w-md space-y-2">
							<Label htmlFor="code" className="flex items-center gap-2 text-sm font-medium">
								<Code className="size-4 text-muted-foreground" />
								Dil kodu
							</Label>
							<div className="relative">
								<Code className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground pointer-events-none" />
								<Input
									id="code"
									value={code}
									onChange={(e) => setCode(e.target.value.trim())}
									placeholder="tr"
									required
									className="pl-9"
									maxLength={20}
								/>
							</div>
						</div>
						<div className="flex flex-wrap items-center gap-3 pt-2 border-t border-border/60">
							<Button type="submit" disabled={updateLanguage.isPending} className="gap-2">
								{updateLanguage.isPending ? (
									<Loader2 className="size-4 animate-spin" />
								) : (
									<Pencil className="size-4" />
								)}
								{updateLanguage.isPending ? "Kaydediliyor..." : "Kaydet"}
							</Button>
							<Button type="button" variant="outline" asChild>
								<Link to={`/languages/${language.id}`}>İptal</Link>
							</Button>
						</div>
					</form>
				</CardContent>
			</Card>
		</div>
	);
}
