...
import { Building, ArrowLeft, MapPin, Phone, Loader2 } from "lucide-react";
		...
		<div className="space-y-8">
			...
			<Card className="w-full border-border/60 shadow-sm overflow-hidden">
				<CardHeader className="border-b border-border/60 bg-muted/30">
					...
					<CardDescription>
						Ad, adres ve telefon bilgilerini girin
					</CardDescription>
				</CardHeader>
				<CardContent className="p-6">
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="flex flex-wrap items-center gap-3 pt-2 border-t border-border/60">
							<Button type="submit" disabled={createOffice.isPending} className="gap-2 bg-brand text-brand-foreground hover:bg-brand/90">
								{createOffice.isPending ? (
									<Loader2 className="size-4 animate-spin" />
								) : (
									<Building className="size-4" />
								)}
								{createOffice.isPending ? "Oluşturuluyor..." : "Oluştur"}
							</Button>
							<Button type="button" variant="outline" asChild>
								<Link to="/offices">İptal</Link>
							</Button>
						</div>
					</form>
				</CardContent>
