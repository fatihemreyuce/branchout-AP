...
function normalizeList(data: unknown): PageTypeResponse[] {
...
export default function PageTypesListPage() {
	...
	const [deleteTarget, setDeleteTarget] = useState<PageTypeResponse | null>(null);
	...
	const handleDelete = () => {
		...
		deletePageType.mutate(deleteTarget.id, {
		...
		<div className="space-y-6">
			...
			<Card className="border-border/60 shadow-sm overflow-hidden">
				<CardHeader className="pb-4 border-b border-border/60 bg-muted/20">
					<CardTitle className="text-lg">Sayfa tipi listesi</CardTitle>
					<CardDescription>
				...
				<CardContent className="p-0">
					{isLoading ? (
						...
						<div className="flex flex-col items-center justify-center py-12 text-muted-foreground gap-2">
							<FileType className="size-10 opacity-50 text-brand" />
							<p>Henüz sayfa tipi yok.</p>
							<Button
								asChild
								variant="outline"
								size="sm"
							...
							>
								<Link to="/page-types/create" className="gap-2">
									<Plus className="size-4" />
									İlk sayfa tipini ekle
								</Link>
							</Button>
						</div>
			...
			<DeleteConfirmDialog
				...
				description={
					deleteTarget
						? `ID ${deleteTarget.id} sayfa tipini silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`
						: undefined
				}
				...
				loading={deletePageType.isPending}
			/>
		</div>
	);
}
