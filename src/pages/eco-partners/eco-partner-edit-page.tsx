...
import { useEcoPartnerById, useUpdateEcoPartner } from "@/hooks/use-eco-partners";
...
export default function EcoPartnerEditPage() {
	...
	const { data: partner, isLoading, isError } = useEcoPartnerById(partnerId);
	const updateEcoPartner = useUpdateEcoPartner();
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const logoPayload: EcoPartnerRequest["logo"] = logoFile ?? (partner?.logo ?? "");
		updateEcoPartner.mutate(
			{ id: partnerId, request: { logo: logoPayload, orderIndex } },
			{ onSuccess: () => navigate(`/eco-partners/${partnerId}`) }
		);
	};
	...
	if (!id || Number.isNaN(partnerId)) {
		return (
			<div className="space-y-6">
				<p className="text-destructive">Geçersiz eco partner ID.</p>
				<Button variant="outline" onClick={() => navigate("/eco-partners")}>
					Listeye dön
				</Button>
			</div>
		);
	...
	if (isError) {
		return (
			<div className="space-y-6">
				<p className="text-destructive">Eco partner bulunamadı veya bir hata oluştu.</p>
				<Button variant="outline" onClick={() => navigate("/eco-partners")}>
					Listeye dön
				</Button>
			</div>
		...
		<div className="space-y-8">
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div className="flex items-center gap-4">
					<Button variant="outline" size="icon" asChild className="shrink-0">
						<Link to={`/eco-partners/${partner.id}`}>
			...
			<Card className="w-full border-border/60 shadow-sm overflow-hidden">
				<CardHeader className="border-b border-border/60 bg-muted/20">
					<CardTitle className="text-lg">Eco Partner bilgileri</CardTitle>
					<CardDescription>
						Logoyu değiştirin veya sıra numarasını güncelleyin
					</CardDescription>
				</CardHeader>
				<CardContent className="p-6">
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="flex flex-wrap items-center gap-3 pt-2 border-t border-border/60">
							<Button
								type="submit"
								disabled={updateEcoPartner.isPending}
								className="gap-2 bg-brand text-brand-foreground hover:bg-brand/90"
							>
								{updateEcoPartner.isPending ? (
									<Loader2 className="size-4 animate-spin" />
								) : (
									<Pencil className="size-4" />
								)}
								{updateEcoPartner.isPending ? "Kaydediliyor..." : "Kaydet"}
							</Button>
							<Button type="button" variant="outline" asChild>
								<Link to={`/eco-partners/${partner.id}`}>İptal</Link>
							</Button>
						</div>
					</form>
				</CardContent>
