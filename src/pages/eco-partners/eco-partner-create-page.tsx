...
import { Leaf, ArrowLeft, Loader2, ImagePlus, ListOrdered } from "lucide-react";
import { useCreateEcoPartner } from "@/hooks/use-eco-partners";
...
export default function EcoPartnerCreatePage() {
	...
	const createEcoPartner = useCreateEcoPartner();
	...
	const handleSubmit = (e: React.FormEvent) => {
		e.preventDefault();
		if (!logoFile) return;
		createEcoPartner.mutate(
			{ logo: logoFile, orderIndex },
			{ onSuccess: () => navigate("/eco-partners") }
		);
	};
		...
		<div className="space-y-8">
					...
					<div className="flex items-center gap-3 min-w-0">
						...
						<div className="min-w-0">
							...
							<p className="text-muted-foreground text-sm mt-0.5">
								Yeni bir eko ortak logosu ekleyin
							</p>
						</div>
					</div>
				</div>
			...
			<Card className="w-full border-border/60 shadow-sm overflow-hidden">
				<CardHeader className="border-b border-border/60 bg-muted/20">
					...
					<CardDescription>
						Logo yükleyin ve listeleme sırasını belirleyin
					</CardDescription>
				</CardHeader>
				<CardContent className="p-6">
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="flex flex-wrap items-center gap-3 pt-2 border-t border-border/60">
							<Button
								type="submit"
								disabled={createEcoPartner.isPending || !logoFile}
								className="gap-2 bg-brand text-brand-foreground hover:bg-brand/90"
							>
								{createEcoPartner.isPending ? (
									<Loader2 className="size-4 animate-spin" />
								) : (
									<Leaf className="size-4" />
								)}
								{createEcoPartner.isPending ? "Oluşturuluyor..." : "Oluştur"}
							</Button>
							<Button type="button" variant="outline" asChild>
								<Link to="/eco-partners">İptal</Link>
							</Button>
						</div>
					</form>
				</CardContent>
