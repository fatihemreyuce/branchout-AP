...
/** API dizi veya { content } döndürebilir; snake_case alanları camelCase'e çevirir */
function normalizeEcoPartnersList(data: unknown): EcoPartnerResponse[] {
...
export default function EcoPartnersListPage() {
	...
	const [deleteTarget, setDeleteTarget] = useState<EcoPartnerResponse | null>(null);
	const { data, isLoading, isError } = useEcoPartners();
	const deleteEcoPartner = useDeleteEcoPartner();

	const handleDelete = () => {
		if (!deleteTarget) return;
		deleteEcoPartner.mutate(deleteTarget.id, {
			onSuccess: () => setDeleteTarget(null),
		});
	};

	const raw = data as Record<string, unknown> | undefined;
	const content = normalizeEcoPartnersList(data);
		...
		<div className="space-y-6">
			...
			<Card className="border-border/60 shadow-sm overflow-hidden">
				<CardHeader className="pb-4 border-b border-border/60 bg-muted/20">
					<CardTitle className="text-lg">Eco Partner listesi</CardTitle>
					<CardDescription>
				...
				<CardContent className="p-0">
					{isLoading ? (
						...
						<div className="flex flex-col items-center justify-center py-12 text-muted-foreground gap-2">
							<Leaf className="size-10 opacity-50 text-brand" />
							<p>Henüz eco partner yok.</p>
							<Button asChild variant="outline" size="sm" className="border-brand-outline text-brand hover:bg-brand-muted">
								<Link to="/eco-partners/create" className="gap-2">
									<Plus className="size-4" />
									İlk eco partner&apos;ı ekle
								</Link>
							</Button>
						</div>
					) : (
						<>
							...
							<ListPagination
								...
								label={`Toplam ${totalElements} eco partner`}
							/>
						</>
					)}
				</CardContent>
			...
			<DeleteConfirmDialog
				...
				description={
					deleteTarget
						? `ID ${deleteTarget.id} (sıra ${deleteTarget.orderIndex}) eco partner kaydını silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`
						: undefined
				}
				...
				loading={deleteEcoPartner.isPending}
			/>
		</div>
	);
}
