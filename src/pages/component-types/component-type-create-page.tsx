		...
		<div className="space-y-8">
					...
					<div className="flex items-center gap-3 min-w-0">
						...
						<div className="min-w-0">
							...
							<p className="text-muted-foreground text-sm mt-0.5">
								Yeni bir bileşen şablonu oluşturun
							</p>
						</div>
					</div>
				</div>
			...
			<Card className="w-full border-border/60 shadow-sm overflow-hidden">
				<CardHeader className="border-b border-border/60 bg-muted/20">
					...
					<CardDescription>
						Tip adı, fotoğraf ve desteklenecek alanları belirleyin
					</CardDescription>
				</CardHeader>
				<CardContent className="p-6">
					<form onSubmit={handleSubmit} className="space-y-6">
							...
							>
								{photoPreview ? (
									<>
										...
										/>
										<p className="text-sm text-muted-foreground">Değiştirmek için tıklayın</p>
									</>
								) : (
									<>
						...
						<div className="flex flex-wrap items-center gap-3 pt-4 border-t border-border/60">
							<Button
								type="submit"
								disabled={createComponentType.isPending || !type.trim()}
								className="gap-2 bg-brand text-brand-foreground hover:bg-brand/90"
							>
								{createComponentType.isPending ? (
									<Loader2 className="size-4 animate-spin" />
								) : (
									<Layout className="size-4" />
								)}
								{createComponentType.isPending ? "Oluşturuluyor..." : "Oluştur"}
							</Button>
							<Button type="button" variant="outline" asChild>
								<Link to="/component-types">İptal</Link>
							</Button>
						</div>
					</form>
				</CardContent>
